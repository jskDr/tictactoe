...
def bellman_v(v:np.ndarray, policy:List, Rsa, Psas, s:int=0, forgetting_factor:float=1.0):
    Gs = 0
    for a in range(len(policy[s])):
        Gs += policy[s][a] * bellman_q_by_v(v, Rsa, Psas, s=s, a=a, forgetting_factor=forgetting_factor)
    return Gs

def bellman_q_by_v(v:np.ndarray, Rsa, Psas, s:int=0, a:int=0, forgetting_factor:float=1.0):
    reward = Rsa[s][a]
    v_next = 0
